...
import uk.ac.cam.js2428.mvqa.ml.FullAttentionVqaDy;
import uk.ac.cam.js2428.mvqa.ml.Mobilenet3by3;

public class FullAttentionDyModel extends CnnLstmModelBase{
    private Mobilenet3by3 cnn;
    private FullAttentionVqaDy model;
    ...
    protected float[] getAnswer() {
        FullAttentionVqaDy.Outputs vqaOutputs = model.process(questionFeature, cnnImageFeature);
        return vqaOutputs.getOutputFeature0AsTensorBuffer().getFloatArray();
    }

    public FullAttentionDyModel(Context context) {
        super(context);
        ...
        try {
            model = FullAttentionVqaDy.newInstance(context, options2);
            questionFeature =
