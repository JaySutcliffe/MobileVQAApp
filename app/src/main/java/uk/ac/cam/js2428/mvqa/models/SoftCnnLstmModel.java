...
public class SoftCnnLstmModel extends CnnLstmModel {
    ...
    protected float[] getAnswer() {
        VqaSoft.Outputs vqaOutputs = model.process(questionFeature, cnnImageFeature);
        return vqaOutputs.getOutputFeature0AsTensorBuffer().getFloatArray();
    }

    public SoftCnnLstmModel(Context context) {
        super(context);
        ...
        try {
            model = VqaSoft.newInstance(context, options2);
            questionFeature =
